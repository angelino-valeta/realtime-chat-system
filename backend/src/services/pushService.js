...
const saveSubscription = async (userId, subscription) => {
  await pool.query(
    'INSERT INTO push_subscriptions (user_id, subscription) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET subscription = $2', [userId, JSON.stringify(subscription)]
  )
}

const sendPushNotification = async (userId, payload) => {
  const result = await pool.query('SELECT subscription FROM push_subscription WHERE user_id = $1', [userId])
